...
<script>
	export default {
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','gequfenlei');
			...
			onAcrossTap(tableName){
     			uni.setStorageSync('crossTable','gequfenlei');
				uni.setStorageSync(`crossObj`, this.detail);
				this.$utils.jump(`../${tableName}/add-or-update?cross=true`);
			},
			// 获取详情
			async init(){
				let res = await this.$api.info('gequfenlei', this.id);
			...
			// 添加评论
			onCommentTap() {
				this.$utils.jump(`../discussgequfenlei/add-or-update?refid=${this.id}`)
			},
			// 点赞
			async zan() {
				let _this = this;
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							...
							await _this.$api.update('gequfenlei', _this.detail);
			...
			async onFinishTap() {
				...
				await this.$api.update('gequfenlei', this.detail);
				this.$utils.msg('审核成功');
